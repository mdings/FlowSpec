...
/**
 * Innermost Flow / Screen / Action name that contains `node`, if any.
 * @param {object} node
 * @returns {string}
 */
function containingStructuralName(node) {
  ...
  while (current) {
    if (
      ...
      const name = String(current.value || "").trim();
      if (name) return name;
    }
 ...
 *   endLine: number,
 *   endColumn: number,
 *   references: Array<{ filePath: string, line: number, statement: string, ref: string, container: string }>
 * }>}
 */
    ...
    walkNodes(origin.root, (node) => {
      ...
      for (const target of matchGoToTargets(ref, targets)) {
        ...
        destination.references.push({
          ...
          statement,
          ref,
          container: containingStructuralName(node),
        });
      }
    });
  }
