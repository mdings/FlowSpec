/**
 * FlowSpec v1 linter — structural rules FS001–FS027 and style warnings FS101–FS108.
 */
...
function lintFileStructure(root, filePath, diagnostics) {
  ...
  walkNodes(root, (node) => {
    ...
    // FS025 / FS026 / FS027 / FS108 Entry rules
    if (node.type === "entry") {
      lintEntry(node, filePath, diagnostics);
    }
    if (node.type === "flow") {
      lintDuplicateEntries(node, filePath, diagnostics);
    }
...
/**
 * FS025 / FS026 / FS027 — Entry ownership, required trigger, and child-free metadata.
 * @param {object} node
 * @param {string} filePath
 * @param {object[]} diagnostics
 */
function lintEntry(node, filePath, diagnostics) {
  if (node.parent?.type !== "flow") {
    diagnostics.push(
      createDiagnostic({
        code: "FS025",
        severity: "error",
        message: "Entry must be a direct child of a Flow.",
        filePath,
        line: node.location.line,
        column: node.location.column,
        suggestion:
          "Move this Entry under the Flow. Entry cannot belong to a Screen, Action, Section, Layout, or behavioral section.",
      })
    );
  }
  ...
  if (!(node.value || "").trim()) {
    diagnostics.push(
      createDiagnostic({
        code: "FS026",
        severity: "error",
        message: "Entry must contain a non-empty trigger.",
        filePath,
        line: node.location.line,
        column: node.location.column,
        suggestion: 'Add a human-readable trigger after Entry, for example "Entry App launch".',
      })
    );
  }
  ...
  if ((node.children || []).length > 0) {
    diagnostics.push(
      createDiagnostic({
        code: "FS027",
        severity: "error",
        message: "Entry cannot own indented children.",
        filePath,
        line: node.location.line,
        column: node.location.column,
        suggestion:
          "Place nested content beside Entry under the Flow. Entry is metadata and does not contain a body.",
      })
    );
  }
}
...
/**
 * FS108 — exact duplicate Entry values on the same Flow.
 * @param {object} flow
 * @param {string} filePath
 * @param {object[]} diagnostics
 */
function lintDuplicateEntries(flow, filePath, diagnostics) {
  /** @type {Map<string, object>} */
  const seen = new Map();
  for (const child of flow.children || []) {
    if (child.type !== "entry") continue;
    ...
    if (first) {
      diagnostics.push(
        createDiagnostic({
          code: "FS108",
          severity: "warning",
          message: `Duplicate Entry "${value}" in this Flow.`,
          filePath,
          line: child.location.line,
          column: child.location.column,
          suggestion: "Use a distinct trigger for each Entry, or remove the duplicate.",
          relatedLocations: [
            {
              message: "First Entry defined here",
              filePath,
              line: first.location.line,
              column: first.location.column,
            },
