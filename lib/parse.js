...
const STRUCTURAL_RE =
  /^(Flow|FLOW|Screen|SCREEN|Action|ACTION|Section|SECTION|Layout|LAYOUT)\b\s*:?\s*(.*)$/;
...
function parseTree(source, filePath = "<stdin>") {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Action-section blocks do NOT use adjacency/same-indent ownership — they must
    // be indented deeper than their owning Screen, Action, or Section.
    if (node.type === "id") {
