...
function parseTree(source, filePath = "<stdin>") {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Action sections at the same indentation as their Action stay under it
    // (common style: Action / Id / Receives all left-aligned).
    if (isSectionType(node.type)) {
      const owner = findSameIndentStructural(stack, indentation, "action");
      if (owner && owner.type === "action") {
        while (
          stack.length > 1 &&
          stack[stack.length - 1] !== owner &&
          stack[stack.length - 1].indentation >= indentation
