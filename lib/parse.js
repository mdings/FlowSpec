 ...
 *
 * Under a Screen, direct named interactions with nested behavior are promoted
 * to Action nodes (implicit Actions) before Id association.
 */
...
const SECTION_RE =
  /^(Receives|Rules|Uses|Steps|Shows|Outcome)\b\s*:?\s*(.*)$/;
...
function parseTree(source, filePath = "<stdin>") {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Sections do NOT use adjacency/same-indent ownership — they must be
    // indented deeper than their owning Screen or Action.
    if (node.type === "id") {
    ...
    // Top-level Flow owns same-indent Screens, Actions, and flow-control.
    if (node.type !== "flow") {
      const flowOwner = findSameIndentStructural(stack, indentation, "flow");
      if (flowOwner && flowOwner.type === "flow") {
        while (
          stack.length > 1 &&
          stack[stack.length - 1] !== flowOwner &&
          stack[stack.length - 1].indentation >= indentation
        ) {
          stack.pop();
        }
        if (flowOwner.indentation === indentation) {
          node.parent = flowOwner;
          flowOwner.children.push(node);
          if (canHaveChildren(node)) stack.push(node);
          continue;
        }
      }
    }
  ...
  // Under a Screen, a direct named interaction (content with a nested body)
  // is treated as an implicit Action before Ids are associated.
  promoteImplicitScreenActions(root);
  associateIds(root);
  return { root, diagnostics, lines };
}

/**
 * Promote Screen-direct content lines that own a deeper-indented body into
 * Action nodes, so they are equivalent to an explicit `Action …` under the Screen.
 *
 * Boundaries:
 * - Only direct children of a Screen (never Flow, Action, or section bodies).
 * - Body must include nested behavior (section, control-flow, Go to, etc.).
 * - Nested prose alone is not promoted (avoids treating ordinary notes as Actions).
 *
 * @param {object} root
 */
function promoteImplicitScreenActions(root) {
  /** @type {object[]} */
  const screens = [];
  walkNodes(root, (node) => {
  ...
  for (const screen of screens) {
    promoteImplicitActionsInScreen(screen);
  }
}
 ...
 * @param {object} screen
 */
function promoteImplicitActionsInScreen(screen) {
...
/**
 * Id belongs to the directly preceding Flow/Screen/Action.
  ...
  walkNodes(root, (node) => {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      for (let j = i - 1; j >= 0; j--) {
        ...
        if (isStructural(prev)) owner = prev;
        break;
      }
      child.owner = owner;
...
function isStructural(node) {
  return node && (node.type === "flow" || node.type === "screen" || node.type === "action");
}
